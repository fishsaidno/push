...
var _validateDocument = function(notification) {
  ...
  check(notification, {
    ...
    androidChannelId: Match.Optional(String),
    ...
    fcm: Match.Optional({
      ...
      // e.g. these top-level fields will be used to populate relevant fcm fields: 'title','text','badge','sound', 'notId', 'contentAvailable', 'androidChannelId', 'payload'
...
Push.send = function(options) {
  ...
  // Rig the notification object
   ...
   // Add extra
   _.extend(notification, _.pick(options, 'payload', 'badge', 'sound', 'notId', 'delayUntil', 'androidChannelId', 'androidPriority', 'iosPriority'));
  ...
  if (Match.test(options.gcm, Object)) {
    notification.gcm = _.pick(options.gcm, 'image', 'style', 'summaryText', 'picture', 'from', 'title', 'text', 'badge', 'sound', 'notId','actions', 'androidChannelId');
  }
