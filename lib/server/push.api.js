...
Push.Configure = function(options) {
  ...
  if (options.gcm && options.gcm.apiKey) {
      ...
      self.sendGCM = function(userTokens, notification) {
          ...
          if(typeof notification.androidChannelId !== 'undefined') {
            data.android_channel_id = notification.androidChannelId;
          }
          else if(Push.debug) {
            console.warn(`For devices running Android 8.0 or later you are required to provide androidChannelId. See https://github.com/raix/push/issues/341 for more info`);
          }
